...
echo 'SSH_CMD="ssh -i $HOME/.ssh/id_copilot -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no -o ServerAliveInterval=15 -o ServerAliveCountMax=2"'
echo ''
echo 'mount_one() {'
echo '    mkdir -p ~/.copilot/$1'
echo '    grep -q "~/.copilot/$1\b\|$HOME/.copilot/$1\b" /proc/self/mountinfo 2>/dev/null && return 0'
echo '    sshfs "${SRC}/$1" ~/.copilot/$1 -o reconnect -o "ssh_command=$SSH_CMD" 2>/dev/null &'
echo '    local pid=$! i=0'
echo '    while [ $i -lt 10 ] && kill -0 $pid 2>/dev/null; do'
echo '        sleep 1; i=$((i+1))'
echo '    done'
echo '    kill -9 $pid 2>/dev/null; return 0'
echo '}'
echo ''
echo '# id_copilot 키로 실제 SSH 인증이 되는지 확인 (port open만 체크하면 sshfs가 패스워드 대기로 hang)'
echo 'hub_reachable() {'
echo '    timeout 6 ssh -qi "$HOME/.ssh/id_copilot" -o BatchMode=yes -o ConnectTimeout=4 -o StrictHostKeyChecking=no "${HUB_USER}@${HUB_HOST}" exit 2>/dev/null'
echo '}'
echo ''
