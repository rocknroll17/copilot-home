...
echo '    sshfs "${SRC}/$1" ~/.copilot/$1 -o reconnect,ssh_command="$SSH_CMD" 2>/dev/null &'
echo '    local pid=$! i=0'
echo '    while [ $i -lt 10 ]; do'
echo '        sleep 1; i=$((i+1))'
echo '        mountpoint -q ~/.copilot/$1 2>/dev/null && return 0'
echo '        kill -0 $pid 2>/dev/null || return 1'
echo '    done'
echo '    kill -9 $pid 2>/dev/null; return 1'
echo '}'
echo ''
...
echo '    else'
echo '        timeout 3 ssh -qo BatchMode=yes -o ConnectTimeout=2 -o StrictHostKeyChecking=no "${HUB_USER}@${HUB_HOST}" exit 2>/dev/null'
echo '    fi'
echo '}'
echo ''
